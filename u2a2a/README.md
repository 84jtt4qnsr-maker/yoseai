...
ヘッダの **📊 使用量**（⚙ 上限の隣）、または 🧵トピック詳細の **使用量内訳** から開く。
発言・レビュー・修正・要約の保存済みmetaを全体／トピック／エージェント別に集計する。
入力・出力・キャッシュトークン（別掲）、累計実行時間（並列分も加算）を表示し、金額は実測USDのみを参考表示する。
欠測は0にせず件数を明示し、定額・費用不明、配送／分岐コピー、未実施を区別する。CLI実行後の失敗は欠測も残す。
Claude／GrokのCLI中断（cancelledかつbilling.mode=unknown）の仮の0は不明として扱う。CodexやGrokの結果イベントにある取得済みusageは保持する。発言フッタでも欠測トークンは「不明」と表示する。

- `public/usage.js` の `aggregateUsage` をブラウザが読み、`lib.mjs` が再exportする（flow-graphと同じ構成）。SSEのstateから直接集計し、追加API・取得キャッシュは持たない。範囲選択中には全体の除外件数を出さない。
- 上限通知は未実施、タスク再入力／cli-syncは外部結果として別途除外する。出所のない旧発言は欠測記録として残す。🧵詳細には既知合計と内訳ボタンを表示する。
- 要約の各試行は `summaryUsage` に保存する。旧履歴は復元できないため `summaryUsageLegacyUnknown` で集計外と示し、旧 `summaryCostUsd` を重ねて加算しない。
- 追加の任意フィールドは旧stateでも読み取れ、初回要約時に初期化する。既存フィールドの意味を変えないためschemaVersionは据え置く。
- ⚙ 上限とは対象が異なる。上限ダイアログの発言実測はコピーを含み、上限判定には要約累計も加算する。予算判定式は変更しない。
- 要約の実測合計を維持するため履歴を件数だけで切り捨てない。保持上限を設ける場合は過去分の集約値も保存する別設計が必要。
