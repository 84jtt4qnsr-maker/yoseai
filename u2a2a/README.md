...
**ローカル専用**: `127.0.0.1` / `localhost` / `[::1]` 以外の `Host` を持つ要求と、
別サイトから送られた要求（`Origin` 不一致・`null`）は本文を読む前に 403 で拒否する
（DNS リバインディングと CSRF の最低限の防御）。`curl` など `Origin` の無い要求は従来どおり通る。
公開ネットワークへ晒す用途は想定していない。

待ち受けは `127.0.0.1` だけ。`localhost` / `[::1]` を許可しているのは**ヘッダの綴り**の話で、
`localhost` が `::1` に解決される環境からは、この検査に届く前に TCP 接続の時点で失敗する
（その場合は `http://127.0.0.1:<ポート>` で開く）。
...
**新しい環境では全員 OFF から始まる** — 最初の発言でいきなり CLI が走って課金されないようにするため。
使う相手をヘッダで ON にしてから始める（既存の `data/state.json` がある環境は、保存されている ON/OFF をそのまま引き継ぐ）。
...
- プロンプト: 「対象プロジェクトは「名前」（パス、閲覧のみ）。Git: ブランチ@HEAD、未コミット変更 N 件」を注入。
  セッション初回のみ README 冒頭＋直下エントリ名の概要（4,000 文字まで）を添える。未紐付けは既定でこのアプリのリポジトリ
